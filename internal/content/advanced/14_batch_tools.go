...
func init() {
	lesson.Register(types.LessonDef{
		...
		Theory: []types.TheoryBlock{
			...
			{Kind: types.Paragraph, Content: "Each tool declares whether it can be batched (run alongside other tools) or must run alone (sequential). Read-only tools like Read, Glob, and Grep are typically batchable. Tools with side effects like Edit, Write, and Bash often must run sequentially."},
			{Kind: types.Code, Content: "  Tool         Policy        Why\n  ────         ──────        ───\n  Read         ⚡ batchable   Read-only, no side effects\n  Glob         ⚡ batchable   Read-only search\n  Grep         ⚡ batchable   Read-only search\n  Edit         🔒 sequential  Modifies files\n  Write        🔒 sequential  Creates/overwrites files\n  Bash         🔒 sequential  Arbitrary side effects"},
