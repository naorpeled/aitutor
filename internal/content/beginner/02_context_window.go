...
func init() {
	lesson.Register(types.LessonDef{
		...
		Theory: []types.TheoryBlock{
			...
			{Kind: types.Bullet, Content: "System prompt — instructions that define the model's behavior\nCLAUDE.md / AGENTS.md — project-specific instructions loaded automatically\nConversation history — all previous messages in the session\nTool results — output from file reads, searches, command execution\nFile contents — code files you're working with"},
			{Kind: types.Heading, Content: "Token Limits"},
			{Kind: types.Paragraph, Content: "Claude's context window is 200,000 tokens — roughly 150,000 words or 500,000 lines of code. While this is enormous, it's not infinite. Large codebases, long conversations, and verbose tool outputs can fill it up."},
			{Kind: types.Code, Content: "  200,000 tokens ≈\n  • 150,000 words\n  • 500,000 lines of code\n  • ~300 average source files"},
		...
		Questions: []types.QuizQuestion{
			...
			},
			{
				...
				Prompt:     "Claude's context window is approximately how many tokens? (enter a number like 200000)",
				Answer:     "200000",
				Explanation: "Claude has a 200,000 token context window, roughly 150,000 words.",
			},
		},
	})
}
