...
func init() {
	lesson.Register(types.LessonDef{
		...
		Theory: []types.TheoryBlock{
			...
			{Kind: types.Paragraph, Content: "Auto memory lets the AI save notes to persistent files that are loaded in future sessions. It stores patterns, conventions, debugging insights, and user preferences — things confirmed across multiple interactions."},
			...
			{Kind: types.Callout, Content: "You can explicitly tell the AI to remember something: \"Always use bun instead of npm\" — and it will persist this preference across sessions."},
		},
		Questions: []types.QuizQuestion{
			{
				Kind:       types.MultipleChoice,
				Prompt:     "Which memory layer is the MOST persistent?",
				Choices:    []string{"Session memory", "Auto memory files", "CLAUDE.md", "Conversation history"},
				CorrectIdx: 2,
				Explanation: "CLAUDE.md is the most persistent — it's version-controlled and always loaded into every session.",
			},
			{
				Kind:       types.MultipleChoice,
