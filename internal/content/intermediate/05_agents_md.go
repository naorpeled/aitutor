...
func init() {
	lesson.Register(types.LessonDef{
		...
		Theory: []types.TheoryBlock{
			...
			{Kind: types.Code, Content: "  ~/.claude/CLAUDE.md          ← user-level (all projects)\n  ~/project/CLAUDE.md          ← project root (whole repo)\n  ~/project/src/CLAUDE.md      ← directory-level (scoped)\n  ~/project/AGENTS.md          ← team-shared (committed)"},
			{Kind: types.Paragraph, Content: "Files cascade: user-level settings apply everywhere, project-level to the whole repo, and directory-level to specific areas. AGENTS.md is the team-shared equivalent meant to be committed to version control."},
		...
		Questions: []types.QuizQuestion{
			{
				...
				Choices:    []string{"CLAUDE.md is for Claude, AGENTS.md is for other AIs", "CLAUDE.md is personal, AGENTS.md is team-shared and committed", "There is no difference", "AGENTS.md is deprecated"},
				CorrectIdx: 1,
				Explanation: "CLAUDE.md is personal configuration, while AGENTS.md is meant to be committed to version control and shared across the team.",
			},
			{
				Kind:       types.MultipleChoice,
